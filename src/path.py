...
from .d2.transformation import vector_rotate_around_origin_ip
...
class Path(Iterable):
    ...
    def create_random_path(self, num_waypoints, min_x, min_y, max_x, max_y):
        ...
        for i in range(num_waypoints):
            ...
            vector_rotate_around_origin_ip(temp, i * spacing)
            temp.x += mid_x
            temp.y += mid_y
            if _ROUNDING:
                temp = round(temp)
...
def _main():
    path = Path(4, 0, 0, 30, 30, True)
    print(path)
    print()

    # for i in range(20):
    #     waypoint = path.current
    #     print(waypoint)
